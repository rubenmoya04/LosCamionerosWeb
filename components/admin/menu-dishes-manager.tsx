...
const AVAILABLE_BADGES = ["Más vendido", "Especialidad", "Tradicional", "Premium", "Postre", "Tapas"]
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              {idEditMode === (formData.id || 0) ? (
                <>
                  <Input type="number" value={tempId} onChange={(e) => setTempId(e.target.value)} className="flex-1" />
...
export default function MenuDishesManager() {
  ...
  const validateAndUpdateId = async () => {
    const newId = Number.parseInt(tempId)
  ...
  const saveDish = async () => {
    ...
    try {
      ...
      const dishData = {
        ...formData,
        id: formData.id ? Number.parseInt(formData.id.toString()) : 0,
      }
            ...
            <Card key={dish.id} className="overflow-hidden hover:shadow-lg transition-shadow">
              <div className="relative aspect-video bg-gray-100 overflow-hidden">
                ...
                <div className="absolute top-3 right-3">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${BADGE_COLORS[dish.badge]}`}>
                    {dish.badge}
                  </span>
                </div>
              </div>
