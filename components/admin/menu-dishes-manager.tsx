...
const BADGE_COLORS: Record<string, string> = {
  ...
  Tapas:
    "bg-gradient-to-r from-lime-600 via-emerald-600 to-green-700 text-white shadow-md shadow-lime-800/30",
};

// Componente Modal para el formulario de plato
function DishModal({ 
  isOpen, 
  onClose, 
  formData, 
        ...
        <div className="p-6 space-y-4">
                ...
                <>
                  ...
                  {!isNewDish && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => startIdEdit(formData.id || 0)}
                    >
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Ej: Pincho camionero"
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <div
              ...
              className={`border-2 border-dashed rounded-lg p-6 text-center transition-colors ${
                dragOverId === (isNewDish ? -1 : formData.id)
            ...
            >
              <input
                ...
                onChange={(e) => {
                  if (e.target.files?.[0]) {
                    handleImageUpload(e.target.files[0], isNewDish ? 0 : (formData.id || 0));
                  }
                }}
                className="hidden"
              ...
              />
              <label
                htmlFor={`image-upload-${isNewDish ? 'new' : formData.id}`}
                className="cursor-pointer block"
              >
                <Upload className="w-6 h-6 mx-auto mb-2 text-gray-400" />
                <p className="text-sm text-gray-600">
                  {uploadingId === (isNewDish ? 0 : formData.id)
                    ? "Subiendo..."
...
export default function MenuDishesManager() {
  ...
  const loadDishes = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log("[v0] Dishes loaded:", data);
        setDishes(data.dishes || data);
      } else {
        toast.error("Error cargando platos");
      }
    } catch (error) {
      console.error("[v0] Error loading dishes:", error);
  ...
  const validateAndUpdateId = async () => {
    ...
    if (newId === editingId || newId === (formData.id || 0)) {
      setIdEditMode(null);
      return;
    }

    const idExists = dishes.some((d) => d.id === newId && d.id !== editingId);
    if (idExists) {
...
¡        <p className="text-muted-foreground">Cargando platos...</p>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Total: {dishes.length} platos | Edita o crea nuevos platos para la carta
          </p>
        </div>
        <Button
            ...
            >
              <div className="relative aspect-video bg-gray-100 overflow-hidden">
                ...
                <div className="absolute top-3 right-3">
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-semibold ${BADGE_COLORS[dish.badge]}`}
                  >
                    {dish.badge}
                  </span>
                </div>
      ...
      {/* Modal para añadir/editar platos */}
      <DishModal
        isOpen={isModalOpen}
        onClose={closeModal}
        formData={formData}
